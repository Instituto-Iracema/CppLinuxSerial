...
//			data.append(buf);
    ...
    int SerialPort::ReadBinary(std::vector<uint8_t> &data, int size)
    {
        data.clear();
        ...
        // This will work because we do not delete/resize the vector while this method
        // is called
        int n = read(fileDesc_, &readBuffer_[0], size);
    ...
    const std::string &SerialPort::device() const
    {
        return device_;
    }

    State SerialPort::state() const
    {
        return state_;
    }
    ...
    #ifdef QT
    QByteArray SerialPort::ReadBinary(int size)
    {
        QByteArray data;
        ...
        // This will work because we do not delete/resize the vector while this method
        // is called
        ssize_t n = read(fileDesc_, &readBuffer_[0], size);
    ...
    void SerialPort::WriteBinary(QByteArray data) {
